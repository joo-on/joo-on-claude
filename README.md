...
```
~/w/mac-cfg (main) [Opus 5 (1M)·xhigh] expl ▓▓▓▓░░░░░░ 42% 14:30
up 3h | $18.80 | +1204/-318 | ⚡5h 100%(1h37m) 7d 41% | cache:91%
tool:Read | agents:2[explore(45s),plan(2m)] | skill:handoff | 3/7
```

1. **Identity** — path, branch, model + reasoning effort, output style abbreviated
   (`expl`, `learn`; hidden when it is `default`), context bar, clock. The model name
   drops the word "context", so `Opus 5 (1M context)` shows as `Opus 5 (1M)`.

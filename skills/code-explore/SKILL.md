---
name: code-explore
description: Analyze a codebase in depth via a shared Phase-1 inventory and 3-5 parallel Explore subagents (scaled to repo size) covering topology, dependencies, abstractions, risks, and testing — then produce a reconciled, evidence-cited Markdown report at the project root. Use ONLY when the user explicitly invokes /code-explore — do not trigger from ambient mentions of exploration or analysis.
...
<!--
  Adapted from toby-plugins (MIT, © Toby Lee)
  Source: https://github.com/tobyilee/toby-plugins
          → plugins/toby-essentials/commands/code-explore.md
  Ported faithfully; future joo-on-claude customizations should preserve
  this header so the provenance remains traceable.

  Note on format change: the upstream ships this workflow as a slash command
  (commands/*.md). joo-on-claude ships it as a skill (skills/<name>/SKILL.md)
  per the current Claude Code guidance that "custom commands have been merged
  into skills" — the invocation surface (/joo-on-claude:code-explore) is
  identical, and `disable-model-invocation: true` preserves the user-only
  trigger semantics of the original command.
-->

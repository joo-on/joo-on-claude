...
<!--
  ...
  Source: https://github.com/tobyilee/toby-plugins
          → plugins/toby-session/skills/handoff/SKILL.md
  ...
  Changes from upstream:
    ...
    3. `git rev-parse --show-toplevel` now falls back to `pwd` so the skill
       cannot write to `/.claude/...` when invoked outside a git repo.
    4. Step 5 confirmation message is English — this plugin keeps templates
       and field labels English-only; document *content* is still written in
       the language of the conversation.
    5. Template's `Model:` example refreshed to a current model.
    6. Background note flags that the cited reference article is external
       and not included in this repo.

  Intentionally preserved:
    - The "handoff is a hypothesis, not a fact sheet" framing — it is the
      whole point of the skill and of its paired `catchup`.
    - Status-not-imperative rule for Open Work, and the Step 4 checklist
      that enforces it before saving.
    - The ~2000 token budget and the no-CLAUDE.md-duplication rule.
-->
...
Use the git root, not `pwd` — matches the rest of this plugin's skills and works from subdirectories. The `|| pwd` fallback only applies outside a git repo, so writes never escape to the filesystem root.
...
> "Handoff written: `.claude/reports/handoff/handoff-{TIMESTAMP}.md`"
...
This skill operationalizes the "Document & Clear" pattern (Tier 2 of the handoff strategy described in `claude-session-context-handoff-4-layer-strategy.md`, an external reference article not included in this repo): before ending a session, dump the session's evolved understanding to a markdown file so a fresh session can pick up from a clean context without re-doing the exploration. Paired skill: `catchup` (reads this document at the start of the next session and verifies its claims).
